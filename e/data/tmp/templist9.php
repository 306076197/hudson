...
?>
               ...
               <div class="aticon">
               	<ul class="huodonglist">
               	   <?php
$bqno=0;
$ecms_bq_sql=sys_ReturnEcmsLoopBq('select titleurl,title,titlepic,smalltext,lastdotime from phome_ecms_news where classid=7 and titlepic!="" order by id desc limit 3',2,24,0);
if($ecms_bq_sql){
while($bqr=$empire->fetch($ecms_bq_sql)){
$bqsr=sys_ReturnEcmsLoopStext($bqr);
$bqno++;
?>
				   <li>
                	<div class="hd_img"><a href="<?=$bqr[titleurl]?>"><img src="<?=$bqr[titlepic]?>" width="180" height="100" /></a></div>
               	    <div class="hd_p">
                   	<h3><span><?=date('Y-m-d', $bqr[lastdotime])?></span><a href="<?=$bqr[titleurl]?>"><?=$bqr[title]?></a></h3>
                      <p><?=$bqr[smalltext]?></p>
                      <div><a href="<?=esub($bqr[titleurl],120)?>">更多 >></a></div>
                    </div>
                   </li>
				   <?php
}
