...
<head>
	...
	<title>[!--pagetitle--] 新款瓷砖,最新全抛釉瓷砖价格,最新微晶石瓷砖价格,最新浴室瓷砖价格,最新背景墙瓷砖价格,最新仿古砖价格,最新木纹砖价格,</title>
...
?>
                ...
                <div class="mright">
                    <div class="picbig">
                        <div class="indexShowt">
                            <div class="topSlide">

                                <div class="slideImg" id="slideImg">
<?php
$bqno=0;
$ecms_bq_sql=sys_ReturnEcmsLoopBq('select morepic from phome_ecms_photo_data_1 where id=5',2,24,0);
if($ecms_bq_sql){
while($bqr=$empire->fetch($ecms_bq_sql)){
$bqsr=sys_ReturnEcmsLoopStext($bqr);
$bqno++;
?>
				                     <?php 
				                       $bqr[morepic] = str_replace('::::::', '', $bqr[morepic]);
                                       $arr = explode('@', $bqr[morepic]);
                                       $i = 0;
                                       $ison = '';
									  foreach($arr as $v){
                                         if($i==0){
                                         ...
                                         //$v = str_replace('rn', '', $v);
                                      ?>
                                      <a href="" title="" target="_blank" style="display: <?=$ison?>;">
										<img width="711" height="360" gid="<?=$i?>" src="<?=$v?>">
										</a>
                                      <?php 
                                      $i++;
                                         }
...
?>
      ...
      <div class="slidenumbg">
         <span gid="1" class="current"></span>
         <?php 
           $j = 2;
           for($j=2;$j<$i;$j++){
           ?>
         ...
         ?>
                        ...
                        <script src="[!--news.url--]skin/default/js/jquery_002.js" type="text/javascript"></script>

                        <script src="[!--news.url--]skin/default/js/slider.js" type="text/javascript"></script>

                    </div>
                    <div class="smpic">
                  
               <div class=slide-pic3 id=slidePic>
				<A class=gray id=prev hideFocus href="javascript:;"></A> 
				<div class=pic-container>
                <ul class="arrlist">
                   <?php
$bqno=0;
$ecms_bq_sql=sys_ReturnEcmsLoopBq('select title,titleurl,titlepic from phome_ecms_photo where classid=10 order by id desc',2,24,0);
if($ecms_bq_sql){
while($bqr=$empire->fetch($ecms_bq_sql)){
$bqsr=sys_ReturnEcmsLoopStext($bqr);
$bqno++;
?>
				     <li>
				       <a href="<?=$bqr[titleurl]?>">
                           <img src="<?=$bqr[titlepic]?>" width="135" height="104" />
                      </a>
                      <p><?=$bqr[title]?></p>
                     </li>
				   <?php
