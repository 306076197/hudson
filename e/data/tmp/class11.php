...
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
	<title>产品分类 哈德逊瓷砖官网|全抛釉瓷砖|微晶石瓷砖|浴室砖瓷砖|背景墙|仿古砖|木纹砖|哈德逊瓷砖-中国领先的品牌瓷砖生产经销商 - www.hudsonhome.cn</title>
...
<div class="cls"></div>
                ...
                <?php
$bqno=0;
$ecms_bq_sql=sys_ReturnEcmsLoopBq('select classid,classname,classpath from phome_enewsclass where bclassid=11 order by classid asc',2,24,0);
if($ecms_bq_sql){
while($bqr=$empire->fetch($ecms_bq_sql)){
$bqsr=sys_ReturnEcmsLoopStext($bqr);
$bqno++;
?>
					   <div class="listbaox">
		                   ...
		                   <div class="pro_b1">
		                            <ul class="list22">
		                              <?php 
		                                $newbook=$empire->query("select title,titleurl,titlepic,smalltext from {$dbtbpre}ecms_news where classid =$bqr[classid] order by id asc");
                                         while($r=$empire->fetch($newbook)) {
					                  ?>
		                                <li><div><a href="<?=$r['titleurl']?>"><img src="<?=$r['titlepic']?>" width="135" height="104" /></a></div><?=$r['smalltext']?></li>
		                               <?php } ?>
		                            </ul>
		                            <div class="cls">
		                            </div>
