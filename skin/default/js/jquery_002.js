...
var td_auto_cycle=function(options){var defaults={}
this.options=$.extend(defaults,options);var _self=this;_self.i=0;_self.is_go_next=true
_self.cycle_length=$(this.options.ControlSel).length;$(this.options.ControlSel).mouseover(function(){$(_self.options.ImgContainerSel).hide();$(this).next(_self.options.ImgContainerSel).eq(0).show();});$(this.options.ImgContainerSel).mouseover(function(){clearInterval(_self.time_interval);});$(this.options.ImgContainerSel).mouseout(function(){clearInterval(_self.time_interval);_self.time_interval=_self.fn_time_interval();});this.fn_time_interval=function(){return setInterval(function(){show_next();},1000);}
function show_next(){var toshow=$(_self.options.ImgContainerSel+":visible").next(_self.options.ImgContainerSel+":hidden:first");$(_self.options.ImgContainerSel+":visible").hide();if(toshow.length>=1){$(toshow).show();}else{$(_self.options.ImgContainerSel+":first").show();}}
this.time_interval=this.fn_time_interval();}
